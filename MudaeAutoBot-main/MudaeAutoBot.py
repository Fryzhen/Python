    ...
    def c(r):
        if r.event.message:
            ...
            if StartwithUser == True:
                return r['author']['id'] == str(mudae) and r['channel_id'] == tide and r['content'].startswith(
                    f"**{bot.gateway.session.user['username']}")
...
def get_server_settings(guild_id, channel_id):
    ...
    FsMsgs = bot.searchMessages(guild_id, channelID=[channel_id], authorID=[bot.gateway.session.user['id']],
                                textSearch=roll_prefix, includeNsfw=True, limit=2)
    FsResults = bot.filterSearchResults(FsMsgs)

    settings_hope_prefix = '$' #TODO
...
def poke_roll(tide):
    logger.debug(
        f"Pokemon Rolling Started in channel {tide}. (If you would like this in a different channel, please configure the desired channel ID as the first in your list)")
    tides = str(tide)
...
def daily_roll(tide):
    logger.debug(
        f"Daily Rolling Started in channel {tide}. (If you would like this in a different channel, please configure the desired channel ID as the first in your list)")
    tides = str(tide)
    if tide not in channel_settings:
...
def waifu_roll(tide, slashed, slashguild):
    ...
    while True:
        ...
        while waifuwait == False:
            ...
            if varwait != None and varwait['content'].startswith(
                    f"**{bot.gateway.session.user['username']}") and "$ku" not in varwait['content']:
                # We over-rolled.
                waifuwait = True
...
def snipe_intent(messagechunk, mreacter, buttonspres):
    if "reactions" in mreacter:
        ...
        elif mreacter["reactions"][0]["emoji"]['id'] != None and "kakera" not in mreacter["reactions"][0]["emoji"][
            "name"]:
...
def is_rolled_char(m):
    embeds = m.get('embeds', [])
    if len(embeds) != 1 or "image" not in embeds[0] or "author" not in embeds[0] or list(
            embeds[0]["author"].keys()) != ['name']:
        # not a marry roll.
        return False
    elif 'footer' in embeds[0] and 'text' in embeds[0]['footer'] and pagination_finder.findall(
...
def on_message(resp):
    ...
    if resp.event.message:
        ...
        if int(channelid) not in channel_settings:
            ...
            logger.error(
                f"Could not find settings for {channelid}, please trigger the '$settings' command in the server and run the bot again.")
            return
        c_settings = channel_settings[int(channelid)]

        if c_settings['pending'] == None and int(aId) != mudae and content[0:c_settings['prefix_len']] == c_settings[
            'prefix'] and content.split(' ')[0][c_settings['prefix_len']:] in mudae_cmds:
            ...
            if not is_rolled_char(m):
                ...
                if m['content'].startswith('<@' + user['id'] + '>') or m['content'].startswith(
                        '<@!' + user['id'] + '>'):
                    # get claim time
            ...
            msg_buf[messageid] = {'claimed': int(embeds[0].get('color', 0)) not in (16751916, 1360437),
                                  'rolled': roller == user['id']}
            ...
            if waifu_wall.get(channelid, 0) != next_claim(channelid)[0]:
                ...
                if str(user['id']) in content:
                    logger.info(
                        f"Wished {charname} from {get_serial(chardes)} with {get_kak(chardes)} Value in Server id:{guildid}")
                    ...
                    if ser in chardes and charcolor == 16751916:
                        ...
                        if "reactions" in m_reacts:
                            if m_reacts["reactions"][0]["emoji"]['id'] == None:
                                bot.addReaction(channelid, messageid, m_reacts["reactions"][0]["emoji"]["name"])
                                break
                            elif m_reacts["reactions"][0]["emoji"]['id'] != None and "kakera" not in \
                                    m_reacts["reactions"][0]["emoji"]["name"]:
                                cust_emoji_sen = m_reacts["reactions"][0]["emoji"]["name"] + ":" + \
                                                 m_reacts["reactions"][0]["emoji"]['id']
                                bot.addReaction(channelid, messageid, cust_emoji_sen)
                                break
                        elif butts.components != []:
                            buttMoji = butts.components[0]["components"][0]["emoji"]["name"]
                ...
                if str(user['id']) not in content and charname.lower() not in chars and get_serial(
                        chardes) not in series_list and int(get_kak(chardes)) < kak_min:
                    logger.debug(
                        f"Ignoring {charname} from {get_serial(chardes)} with {get_kak(chardes)} Kakera Value in Server id:{guildid}")
            if butts.components != []:
    ...
    if resp.event.reaction_added:
        ...
        if int(rchannelid) not in channel_settings:
            ...
            logger.error(
                f"Could not find settings for {rchannelid}, please trigger the '$settings' command in the server and run the bot again.")
            return
        ...
        if reactionid == mudae and int(rchannelid) in mhids:
            ...
            if emojiid != None and emoji.lower() in KakeraVari:
                ...
                if cooldown <= 1:
                    logger.info(
                        f"{emoji} was detected on {react_m['author']['name']}:{get_serial(react_m['description'])} in Server: {rguildid}")
                    time.sleep(snipe_delay)
                    bot.addReaction(rchannelid, rmessageid, sendEmoji)
                else:
                    logger.info(
                        f"Skipped {emoji} found on {react_m['author']['name']}:{get_serial(react_m['description'])} in Server: {rguildid}")
                    return

                warn_check = mudae_warning(rchannelid)
                kakerawallwait = wait_for(bot, lambda r: warn_check(r) and 'kakera' in r.parsed.auto()['content'],
                                          timeout=5)
